		...
		for(int j = 1; j <= (allo.K+1)/2; j++){
			...
			for(int k = 0; k < curC.size();k++){
				for(int l = 0; l< allo.nbNodes;l++){		
					...
					if(l != i && (curCost[k] + potNC < min(allo.B,1) || (curCost[k] + potNC == min(allo.B,1) && flop[l][i] <= allo.K - j))){
						bool add = true;
						for(int m = 1; m <= j -1; m++){
							if(curC[k][m] == l){
								add = false;
	...
	for(int i = 0; i < allo.nbNodes;i++){
		for(int j = i+1; j < allo.nbNodes;j++){
			for(int k = 1; k < allo.K; k++){
				if(khcycles[i][j][k].size() > 0 && khcycles[j][i][k].size() + khcycles[j][i][k-1].size() > 0 && khcyclesMinC[i][j][k] + min(khcyclesMinC[j][i][k], khcyclesMinC[j][i][k-1]) <= min(allo.B,1)){
