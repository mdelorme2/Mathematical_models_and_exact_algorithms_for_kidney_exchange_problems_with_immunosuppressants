...
void Allocation::load(const string& path, const string& filein){
	...
	if (file){
		...
		for(;;){
			...
			iss.clear(); 
			getline(file, parser);
			parser.erase(remove_if(parser.begin(), parser.end(), ::isspace), parser.end());
			if (parser == "\"altruistic\":true,") {
				nbA++;
				iss.str(parser); iss.clear();
			}				
			else{
				nbP++;
				iss.str(parser); iss.clear(); 
...
void Allocation::printProb(){
	...
	for(int i = 0; i < nbNodes;i++){
		if(i < nbP){
			...
			cout << "Altruistic " << xti[i] << " :";
			for(int j = 0; j < adj[i].size();j++)
				cout << " " << xti[adj[i][j]];
			cout << endl;
		}
	}
	cout << "K is " << K << " and L is " << L << " and B is " << B << endl;
}
...
void Allocation::printInfo(const string& pathAndFileout){
	...
	file << infos.LB << "\t" << infos.UB <<  "\t" << infos.nbVar << "\t" << infos.nbCons << "\t" << infos.nbNZ << endl;
	file.close();
}