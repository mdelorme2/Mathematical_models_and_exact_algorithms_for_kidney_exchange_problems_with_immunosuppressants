...
int cycle(Allocation& allo){
	...
	for(int k = allo.nbP; k < allo.nbNodes;k++){
		tails[k] = true;
		arcsC.push_back({k,0,-1,-1});
		// cout << "ADD " << allo.xti[k] << " " << 0 << " " << -1 << " " << -1 << endl;
	}
	for(int j = 0; j < allo.L - 1; j++){
		// Add initial ascending reserve arcs
		if(j <= allo.L - allo.K - 2){
			for(int k = 0; k < allo.nbNodes;k++){
				if(tails[k]){
					arcsC.push_back({k,j,allo.nbNodes,j+1});
					// cout << "ADD " << allo.xti[k] << " " << j << " " << "Blank" << " " << j + 1 << endl;
				}
			}
		}
		vector<bool> tailsN(allo.nbP,false);
		...
		for(int k = 0; k < allo.edges.size();k++){
			if(tails[allo.itx[allo.edges[k][0]]]){
				tailsN[allo.itx[allo.edges[k][1]]] = true;
				arcsC.push_back({allo.itx[allo.edges[k][0]],j,allo.itx[allo.edges[k][1]],j+1});
				// cout << "ADD " << allo.edges[k][0] << " " << j << " " << allo.edges[k][1] << " " << j + 1 << endl;
			}
		}
		// Add closing arcs
		for(int k = 0; k < allo.nbP;k++){
			if(tailsN[k]){
				arcsC.push_back({k,j+1,-1,-1});
				// cout << "ADD " << allo.xti[k] << " " << j+1 << " " << "-1" << " " << "-1" << endl;
			}
		}
		// Add descending reserve arcs
		if(j <= allo.L - allo.K - 2){
			for(int k = 0; k < allo.nbP;k++){
				tailsN[k] = true;
				arcsC.push_back({allo.nbNodes,j+1,k,j+1});
				// cout << "ADD " << "Blank" << " " << j + 1 << " " << allo.xti[k]  << " " << j+1 << endl;
			}
		}
		// Update the reachable nodes
		tails = tailsN;
	...
	try{
		...
		vector<vector<GRBLinExpr> > fiC(allo.nbNodes + 1, vector<GRBLinExpr>(allo.L,0));
		vector<vector<GRBLinExpr> > foC(allo.nbNodes + 1, vector<GRBLinExpr>(allo.L,0));
		vector<vector<bool> > fbC(allo.nbNodes + 1, vector<bool>(allo.L,false));
