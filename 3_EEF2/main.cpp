...
int EEF(Allocation& allo){
		...
		for(int j = 0; j < allo.K; j++){
			for(int k1 = i; k1 < allo.nbNodes;k1++){
				for(int k2 = i; k2 < allo.nbNodes;k2++){
					...
					if(j > 0 && k1 == i) continue;
					if(tails[j][k1] + (1 - allo.adm[k1][k2]) + heads[j][k2] <= min(allo.B,1)){
	...
	try{
		...
		GRBLinExpr objFun = 0;
		GRBLinExpr costE = 0;
		vector<GRBLinExpr> costEPG(allo.nbNodes,0);
		...
		for (int i = 0; i < allo.nbNodes; i++){
			for (int j = 0; j < anedges[i].size(); j++){
				...
				costEPG[i] += (1 - allo.adm[anedges[i][j][0]][anedges[i][j][1]]) * isEdgeUsed[i][j];
			}
		}
		...
		for (int i = 0; i < allo.nbNodes; i++){
			...
			model.addConstr(costEPG[i] <= allo.B * fo[i][i]);
		}
